...
ValueAst read_factor(Source src) {
   ...
   switch (tok.type) {
	 ...
	 return new VariableAst(tok.str);
...
Ast read_stmt(Source src) {
   ...
   switch (kw.type) {
	 ...
	 auto eq = src.get;
	 if (!eq || eq.str != "=") {
	    throw new Exception("= is required. line %d".format(src.line));
	 }

	 auto init = src.read_expr;
	 if (!init) {
	    throw new Exception("Initializer is required. line %d".format(src.line));
	 }
